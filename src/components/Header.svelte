<script lang="ts">
	...
	let { session }: { session: Awaited<ReturnType<typeof getSession>> } = $props();
	...
	async function updateUser() {
		await tr(async () => {
			...
			// console.log(t);
			if (!r.user) {
				timeLeft = Math.max(DAY - (+new Date() - +new Date(user.dailyClaimedAt!)), 0);
				if (timeLeft == 0) {
				...
				{:else}
					<button
						...
						onclick={async () => {
							...
							await tr(async () => {
								await trpc($page).roll.query();
								await updateUser();
								let cards = await trpc($page).mycards.query();
								setCards(cards);
							});
						}}
					>
...
<div bind:this={loader} class="loader-container w-full h-2 overflow-hidden relative hidden">
